...
write.csv(
  ...
  file.path(OUT_DECONV_DIR, sprintf("excluded_samples_genomic_high_na_%s_12.csv", probe_library)),
  row.names = FALSE
)
...
out_file <- file.path(
  OUT_DECONV_DIR,
  sprintf("blood_cell_fractions_%s_genomic_batch_corrected_12.rds", probe_library)
)
...
csv_file <- file.path(
  OUT_DECONV_DIR,
  sprintf("blood_cell_fractions_%s_genomic_batch_corrected_12.csv", probe_library)
)
